...
You are talking with them, not filling in a form.

Answer only from the call record below. Be brief, two or three sentences at most.
If the record does not contain the answer, say so plainly. Never invent details about the
caller, the appointment, or what was said.

If they say something conversational rather than asking a question, like "ok", "thanks" or
"got it", just reply naturally and briefly. Do not tell them their question is unclear and
do not ask them to rephrase. Offer one useful thing you could tell them about this call
instead.`;
...
export async function ask(call, actions, question, history = []) {
  const prior = history
    .slice(-6)
    .map((t) => `${t.role === "you" ? "OWNER" : "YOU"}: ${t.text}`)
    .join("\n");
