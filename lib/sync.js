...
export async function syncFromElevenLabs() {
  ...
  for (const c of conversations) {
    ...
    let live = !DONE.has(String(detail.status || "").toLowerCase());
    // A call stuck in a non-final state with nothing said is stale, not live.
    // Unanswered outbound attempts sit at "initiated" forever.
    const ageMs = Date.now() - startedAt;
    if (live && ageMs > 3 * 60_000) live = false;
